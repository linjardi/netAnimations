...
var stp = 0;
var k = 0;
const DELAYTIME = 3000; //Tempo em milisegundos de delay para leitura do navegador de tela não sobrepor o audio
...
Animator.prototype.speak = function(){
	...
	if (aud){
		...
		if (buttonSpeak == 1){
			time = DELAYTIME;
			setTimeout(function(){sound.play();time = 0;}, time);
		}else{
			sound.play();
		}
...
Animator.prototype.nextStep = function() {
	  ...
	  var prox = false;
	  if (this.getStatus() == AnimationStatus.CONTINUE) {
			//verifica se é a primeira vez que o botão está sendo utilizado
			if(!back){
				if(!backAndBlock){
					qtdnext++;
					qtdback=0;
			...
			if(qtdnext == 1 || qtdback == 1){
				if (aud){
					time = DELAYTIME; // 3 segundos para o leitor de tela ler o que é o botão selecionado
				}
			}
...
function ImageHost(imgElement, coord, message, dWidth, dHeight, sWidth) {
	...
	this.velocityX = 0;
	this.img = new Image();
	this.img.src = "../img/elements2.png";
	this.opacity = 0;
...
var MessageAnimation2 = function(pos, next, routers, innerSound) {
...
//threeWayhandShake Animation

ImageElementsThreeWayhandShake = Object.freeze({
	"HOST" : { name:"host", dx: 0, textDy: 25 },
	"SERVER" : { name:"server", dx: 100, textDy: 10 },
	"ROUTER": { name: "router", dx: 200, textDy: 25 },
	"SEGMENTO": { name: "segmento", dx: 700, textDy: -85 },
	"SEGMENTO2": { name: "segmento2", dx: 800, textDy: -85 },
	"SEGMENTO3": { name: "segmento3", dx: 900, textDy: -85 },
...
var ThreeWayhandShakeConf = {
	...
	"paths": [{"sx":200, "sy":370, "dx":450, "dy":370},
				    ...
				    {"sx":650, "sy":370, "dx":860, "dy":370},
			     ],

	"text": [{"text": "IP 130.57.20.1", "coord": {"x":200, "y":450}},
					 {"text": "Servidor Destino", "coord": {"x":200, "y":475}},
					 {"text": "IP 194.153.205.26", "coord": {"x":900, "y":450}},
				   {"text": "Cliente Origem", "coord": {"x":900, "y":475}}],
}
...
ThreeWayhandShakeAnimation.prototype.render = function(ctx) {
	//opacity effect
	ctx.globalAlpha = this.prototype.opacity;
	//caminhos
	this.drawPaths(ctx, this.paths);
...
ThreeWayhandShakeAnimation.prototype.drawPaths = function(ctx, paths) {
	//caminhos
	for (var i = 0; i < paths.length; i++) {
		this.drawPath(ctx, paths[i]);
	}
...
ThreeWayhandShakeAnimation.prototype.update = function() {
	if (this.prototype.opacity <= 1) {
		this.prototype.opacity += 0.1;
	} else {
		this.prototype.stop();
...
ThreeWayhandShakeAnimation.prototype.drawPath = function(ctx, coord) {
	ctx.beginPath();
	ctx.lineWidth=(coord.line) ? (coord.line) : "10";
	ctx.strokeStyle= (coord.color) ? coord.color : "#999"; // Green path
	ctx.moveTo(coord.sx, coord.sy);
...
ThreeWayhandShakeAnimation.prototype.drawText = function(ctx, text) {
	ctx.save();
	ctx.font = APP.CONF.getTextStyle(text.style);
	ctx.fillStyle = (text.color) ? text.color : "#000";
	ctx.textAlign = 'center';
...
var TwhsIncrease = function(conf, msg) {
	this.prototype = new Animation();
	this.twhsAnimation = new ThreeWayhandShakeAnimation(conf, msg);
	this.html = new HtmlContentAnimation(msg);
	//this.prototype.bgClear = true;
	this.seg = conf.img;
	this.seg[0].setDWidth(50);
	this.seg[0].setDHeight(50);
	this.seg[0].setDy(320);
	this.prototype.next = true;
};

TwhsIncrease.prototype.render = function(ctx) {
...
HtmlMessageAnimation2.prototype.render = function(ctx) {
	ctx.globalAlpha = 1;
	for (var i = 0; i < this.img.length; i++) {
		this.img[i].drawImage(ctx);
	}
	for(var i in this.elements) {
		APP.newElement(this.elements[i]);
	}
...
var TwhsDecreaseLeft = function(conf, msg, innerSound) {
	this.prototype = new Animation();
...
var TwhsDecreaseRight = function(conf, msg, innerSound) {
	this.prototype = new Animation();
	this.hosts = conf.hosts;
	this.server = conf.server;
	this.router = conf.router;
...
TcpAnimation.prototype.render = function(ctx) {
	//opacity effect
	ctx.globalAlpha = this.prototype.opacity;
	//caminhos
	this.drawPaths(ctx, this.paths);
...
TcpAnimation.prototype.drawPaths = function(ctx, paths) {
	//caminhos
	for (var i = 0; i < paths.length; i++) {
		this.drawPath(ctx, paths[i]);
	}
...
TcpAnimation.prototype.update = function() {
	if (this.prototype.opacity <= 1) {
		this.prototype.opacity += 0.1;
	} else {
		this.prototype.stop();
...
TcpAnimation.prototype.drawPath = function(ctx, coord) {
	ctx.beginPath();
	ctx.lineWidth=(coord.line) ? (coord.line) : "10";
	ctx.strokeStyle= (coord.color) ? coord.color : "#999"; // Green path
	ctx.moveTo(coord.sx, coord.sy);
...
var EthernetAnimation = function(animationConf, msgToShow, bgClear, next) {
	this.prototype = new Animation(msgToShow);
	this.hosts = animationConf.hosts;
	this.paths = animationConf.paths;
	this.text = animationConf.text;
...
EthernetAnimation.prototype.render = function(ctx) {
	//opacity effect
	ctx.globalAlpha = this.prototype.opacity;
	//caminhos
	this.drawPaths(ctx, this.paths);
...
EthernetAnimation.prototype.drawPaths = function(ctx, paths) {
	//caminhos
	for (var i = 0; i < paths.length; i++) {
		this.drawPath(ctx, paths[i]);
	}
...
EthernetAnimation.prototype.update = function() {
	if (this.prototype.opacity <= 1) {
		this.prototype.opacity += 0.1;
	} else {
		this.prototype.stop();
...
EthernetAnimation.prototype.drawPath = function(ctx, coord) {
	ctx.beginPath();
	ctx.lineWidth=(coord.line) ? (coord.line) : "10";
	ctx.strokeStyle= (coord.color) ? coord.color : "#999"; // Green path
	ctx.moveTo(coord.sx, coord.sy);
...
EthernetAnimation.prototype.drawText = function(ctx, text) {
	ctx.save();
	ctx.font = APP.CONF.getTextStyle(text.style);
	ctx.fillStyle = (text.color) ? text.color : "#000";
	ctx.textAlign = 'center';
...
var EthernetAnimationTransition = function(animationConf, msgToShow, persistent, next) {
	this.prototype = new Animation(msgToShow);
	this.hosts = animationConf.hosts;
	this.paths = animationConf.paths;
	this.text = animationConf.text;
...
EthernetAnimationTransition.prototype.render = function(ctx) {
	//opacity effect
	ctx.globalAlpha = this.prototype.opacity;
...
EthernetAnimationTransition.prototype.update = function() {
	if (this.prototype.opacity <= 1) {
		this.prototype.opacity += 0.1;
	} else {
		this.prototype.stop();
...
var MessageAnimationImgAndShine = function(pos, next, imgs, innerSound) {
this.prototype = new Animation();
this.msgs = this.buildMsgs(pos);
this.prototype.persistent = false;
this.opacity = 0.5;
...
MessageAnimationImgAndShine.prototype.buildMsgs = function(pos) {
var msgs = [];
for (var i = 0; i < pos.length; i++) {
	msgs[i] = new Message(pos[i]);
}
...
MessageAnimationImgAndShine.prototype.render = function(ctx) {
	//drawShine

	//alert(this.step);
	if(this.stepOn && this.step < 30){
		this.drawLed(ctx);
		this.step--;
		if(this.step == 0){
			this.stepOn = !this.stepOn;
		}
	}else{
		this.step++;
		if(this.step == 29){
...
MessageAnimationImgAndShine.prototype.drawLed = function(ctx){
		for (var j = 0; j < this.img.length; j++) {
			this.img[j].drawImage(ctx);
		}
};

MessageAnimationImgAndShine.prototype.update = function() {
...
var MessageAnimationImgAndSignal = function(pos, next, imgs) {
this.prototype = new Animation();
this.msgs = this.buildMsgs(pos);
this.prototype.persistent = false;
this.opacity = 0.5;
...
MessageAnimationImgAndSignal.prototype.buildMsgs = function(pos) {
var msgs = [];
for (var i = 0; i < pos.length; i++) {
	msgs[i] = new Message(pos[i]);
}
...
MessageAnimationImgAndSignal.prototype.render = function(ctx) {
	//drawShine
...
MessageAnimationImgAndSignal.prototype.drawLed = function(ctx){
		for (var j = 0; j < this.img.length; j++) {
			this.img[j].drawImage(ctx);
		}
};

MessageAnimationImgAndSignal.prototype.update = function() {
...
var Movement = function(animationConf, msg, type, vectorD, innerSound) {
	this.prototype = new Animation();
	this.hosts = animationConf.hosts;
	this.server = animationConf.server;
	this.paths = animationConf.paths;
	...
	this.prototype.next = true;
	this.prototype.bgClear = true;
	this.prototype.persistent = true;
	time = 0;
	this.innerSound = innerSound;
		var s = this.innerSound;
		new som(s); //Load a new sound
...
var BlockAnimation = function(stp, type) {
	this.prototype = new Animation();
	this.stp = stp;
	this.type = type;
	this.coords = [];
...
BlockAnimation.prototype.addSteps = function(){

	var realPassos = this.stp;
	...
	while (this.stp > 0){
		...
		if (this.type == 1){
			...
			newSteps.push(new MessageAnimation2(c, m, n, s));
		}else if(this.type == 3){
			newSteps.push(new MessageAnimationImg(c, m, n, s));
		}else if(this.type == 4){
			newSteps.push(new TwhsIncrease(c, m));
		}else if(this.type == 5){
			newSteps.push(new TwhsDecreaseLeft(c, m, n));
		}else if(this.type == 6){
			newSteps.push(new TwhsDecreaseRight(c, m, n));
		}else if(this.type == 7){
			newSteps.push(new MessageAnimationImgAndShine(c, m, n, s));
		}else{
			newSteps.push(new Movement(c,m,n,s,a));
		}
...
var BlockBackground = function(stp) {
	this.prototype = new Animation();
	this.stp = stp;
	this.type = [];
	this.msgs = [];
...
BlockBackground.prototype.addSteps = function(){
	...
	while (this.stp > 0){
		...
		if(t == 'ti'){ //TwhsIncrease
			newSteps.push(new TwhsIncrease(n,m));
		}
		if(t == 'ta'){ //ThreeWayhandShakeConf
			newSteps.push(new ThreeWayhandShakeAnimation(o,m,o2,n));
		}
		if(t == 'eta'){ //EthernetAnimation
			newSteps.push(new EthernetAnimation(o,m,o2,n));
		}
...
BlockBackground.prototype.setAttributes = function(type,msgs,next,outros,outros2) {
	this.type.push(type);
	this.msgs.push(msgs);
	this.nexts.push(next);
...
function goHome(){
	window.location.href = "http://ws2.din.uem.br/~animanet/";
}
